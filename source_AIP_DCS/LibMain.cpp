...
oPlaneData ChangeData(int ID, int Team, float HP, int OperType, oNavigationData& NaviData)
{
    ...
    // 2026-08-17 버그 수정: 기존 코드는 위경도(도 단위)를 변환 없이 그대로
    // LocationX/Y에 넣어서, 이후 모든 Cartesian 거리/방향 계산이 사실상 고도차만
    // 반영하는 잘못된 값이 됐다(실측: 실제 3783m 떨어진 두 기체가 BB->Distance로는
    // 31m로 나옴). 이미 있던 LLAtoCartesian()을 고정 기준점(라인 297의 GetStick
    // 경로와 동일한 지도 원점)으로 호출해서 진짜 로컬 평면좌표(미터)로 변환한다.
    BT_Geometry::Vector3 Cartesian = LLAtoCartesian(LLA, BT_Geometry::Vector3(37.91455691666666, 128.18188127777776, 0));
    PlaneData.LocationX = Cartesian.X;
    PlaneData.LocationY = Cartesian.Y;
    PlaneData.LocationZ = Cartesian.Z;
