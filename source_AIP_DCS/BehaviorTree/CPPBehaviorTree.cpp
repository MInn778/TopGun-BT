...
void UCPPBehaviorTree::init()
{
	...
	try
	{
		...
		*/
		...
		Factory.registerNodeType<Action::Task_EmergencyPullUp>("Task_EmergencyPullUp");
		Factory.registerNodeType<Action::Task_WEZAttack>("Task_WEZAttack");
		Factory.registerNodeType<Action::Task_MidRangeEngage>("Task_MidRangeEngage");
		Factory.registerNodeType<Action::Task_LongRangeApproach>("Task_LongRangeApproach");
		Factory.registerNodeType<Action::Task_Evade>("Task_Evade");
		Factory.registerNodeType<Action::Task_Intercept>("Task_Intercept");	// 2026-08-18: 팀원(YUGKUN02) BT의 등속 원운동 닫힌형 요격 예측 노드 병합
		Factory.registerNodeType<Action::DECO_ThreatCheck>("DECO_ThreatCheck");
		Factory.registerNodeType<Action::DECO_EmergencyGuard>("DECO_EmergencyGuard");



		//파일로 트리 구조 정의
		// 2026-08-22: v2(AIP_DCS_v2)와 같은 파일(Rule_forTraining.xml)을 공유하고
		// 있었음 -- v2 개발 중 그 파일 내용이 계속 바뀌면서 v1이 자기도 모르게
		// v2의 최신(때론 미검증) 트리 설정을 그대로 로드하는 위험이 있었음(과거
		// 한 번 이 파일이 빈 스켈레톤으로 방치돼서 한 달간 라이브가 그 위에서
		// 돌았던 사고도 있었음). v1 전용 파일로 분리한다.
		tree = Factory.createTreeFromFile("./Rule_v1.xml");
